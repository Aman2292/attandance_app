                  ...
                  children: [
                    ValueListenableBuilder<DateTime>(
                      ...
                      builder: (context, currentMonth, child) {
                        return Text(
                            'Upcoming Holidays (${DateFormat('MMMM yyyy').format(currentMonth)})',
                          style: TextStyle(
                      ...
                      builder: (context, holidays, child) {
                        return SfCalendar(
                          ...
                          monthCellBuilder: (BuildContext context, MonthCellDetails details) {
                            ...
                            final isCurrentMonth = _currentMonth.value.year == details.date.year && _currentMonth.value.month == details.date.month;
                            final isToday = isCurrentMonth && now.year == details.date.year && now.month == details.date.month && now.day == details.date.day;
                            Color? bgColor;
...
// Holiday Data Source with Custom Colors
class HolidayDataSource extends CalendarDataSource {
  HolidayDataSource(List<Map<String, dynamic>> holidays) {
    appointments = holidays.map((h) {
      Color color;
      if (h['icon'] == '🇮🇳') color = Colors.blue; // National holidays
      else if (h['icon'] == '🎉' || h['icon'] == '🌊') color = Colors.orange; // Festivals
      else color = Colors.red; // Default
      return Appointment(
        startTime: h['date'],
